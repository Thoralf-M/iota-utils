<script lang="ts" module>
    ...
    let intialClientConfig = JSON.parse(localStorage.getItem('clientConfig')!) || defaultConfig;
    let clientConfig: ClientConfig = $state(intialClientConfig);
    let selectedNetwork: any = $state(intialClientConfig.selected);
    let jsonClientConfigString = $state(JSON.stringify(intialClientConfig, null, 2));
    ...
    function handleInput() {
        try {
            ...
            if (
                ...
                let exists = clientConfig.networks.find((a) => a.name == clientConfig.selected);
    ...
    export function getSelectedNetwork(): Network {
        return clientConfig.networks.find((network) => network.name == selectedNetwork)!;
    }
...
<main>
    ...
    {#if $showSettings}
        <div>
            <textarea bind:value={jsonClientConfigString} oninput={handleInput} rows="12" cols="100"
            ></textarea>
        </div>
        <button
            onclick={() => {
