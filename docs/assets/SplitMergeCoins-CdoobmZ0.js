...
function SplitMergeCoins($$anchor, $$props) {
	...
	const mergeAllIotaCoins = async () => {
		try {
			let coins = await getAllIotaCoins(getClient(), $activeAddress());
			if (coins.length < 2) throw new Error("No coins to consolidate");
	...
	const splitIotaCoins = async () => {
		try {
			...
			while (remaining > 0) {
				...
				const commandIndex = tx.splitCoins(tx.gas, splitAmounts)[0].NestedResult[0];
				const chunkCoinArgs = Array.from({ length: chunkSize }, (_, i) => ({
					kind: "NestedResult",
					NestedResult: [commandIndex, i]
				}));
	...
	const listAllIotaCoinObjects = async () => {
		try {
			let coins = await getAllIotaCoins(getClient(), $activeAddress());
			set(iotaBalance, 0);
			for (const coin of coins) set(iotaBalance, get(iotaBalance) + parseInt(coin.balance));
...
delegate(["click"]);
//#endregion
export { SplitMergeCoins as default };
