...
import { _ as nanoToIotaFormatted, m as formatNumbersWithUnderscores } from "./index-115pSspU.js";
...
async function fetchLatestSystemState(client) {
	...
	const baseFields = {};
	for (const [key, val] of Object.entries(systemState)) if (!skipKeys.has(key)) baseFields[key] = val;
	...
	for (const v of systemState.activeValidators) validatorByAddress.set(v.iotaAddress, v);
	const committeeRows = [];
	...
	for (const v of systemState.activeValidators) if (!committeeAddresses.has(v.iotaAddress)) activeValidatorRows.push(extractActiveValidatorRow(v));
	return {
...
async function fetchCandidateValidators(client, showAllValidatorData) {
	...
	if (!validatorCandidatesId || validatorCandidatesId === "") return {
	...
	while (hasNextPage) {
		...
		for (const candidateValidator of candidateValidatorsPage.data) {
			...
			stakeInfo.candidateValidatorsStake += parseInt(validator.staking_pool.fields.iota_balance);
			validatorRows.push(extractValidatorRow(validator));
			if (!showAllValidatorData) cleanupValidatorFields(validator);
	...
	return {
		formattedValidators: validatorCandidates.length > 0 ? formatNumbersWithUnderscores(validatorCandidates) : "No candidate validators",
		stakeInfo: formatNumbersWithUnderscores(stakeInfo),
		validatorRows
	};
}
async function fetchPendingValidators(client, showAllValidatorData) {
	...
	while (hasNextPage) {
		...
		for (const pendingValidator of pendingValidatorsPage.data) {
			...
			stakeInfo.pendingValidatorsStake += parseInt(validator.staking_pool.fields.iota_balance);
			validatorRows.push(extractValidatorRow(validator));
			if (!showAllValidatorData) cleanupValidatorFields(validator);
			pendingValidators.push(validator);
		}
		hasNextPage = pendingValidatorsPage.hasNextPage;
		if (hasNextPage) nextPageCursor = pendingValidatorsPage.nextCursor;
	}
	return {
		formattedValidators: pendingValidators.length > 0 ? formatNumbersWithUnderscores(pendingValidators) : "No pending validators",
		stakeInfo: formatNumbersWithUnderscores(stakeInfo),
		validatorRows
	};
}
async function fetchInactiveValidators(client, showAllValidatorData) {
	...
	if (parseInt(size) === 0) return {
		formattedValidators: "No inactive validators",
		validatorRows: []
	};
	...
	while (hasNextPage) {
		...
		for (const inactiveValidator of inactiveValidatorsPage.data) {
			...
			const validatorV1 = await client.getDynamicFields({ parentId: inactiveValidatorsPage.data.content.fields.value.fields.inner.fields.id.id });
			const validator = (await client.getObject({
				id: validatorV1.data[0].objectId,
				options: { showContent: true }
			})).data?.content.fields.value.fields;
		...
		if (hasNextPage) nextPageCursor = inactiveValidatorsPage.nextCursor;
	}
	...
	validatorRows.sort((a, b) => {
		...
		return parseInt(b.stakingPoolDeactivationEpoch ?? "0") - aEpoch;
	});
	return {
		formattedValidators,
		validatorRows
...
//#endregion
//#region src/lib/pages/iota-system-state/ValidatorTable.svelte
var root$1 = from_html(`<th class="sortable right svelte-7dv5n8">Eff. Commission (<a href="https://github.com/iotaledger/IIPs/blob/main/iips/IIP-0008/IIP-0008.md" target="_blank" rel="noopener noreferrer" class="svelte-7dv5n8">IIP-8</a> </th>`);
var root_1$1 = from_html(`<th class="sortable right svelte-7dv5n8"> </th>`);
var root_2$1 = from_html(`<td class="right svelte-7dv5n8"> </td>`);
var root_3$1 = from_html(`<td class="mono right svelte-7dv5n8"> </td>`);
var root_4$1 = from_html(`<tr><td class="name svelte-7dv5n8"> </td><td class="right svelte-7dv5n8"> </td><!><td class="mono right svelte-7dv5n8"> </td><!><!><!><!><td class="mono svelte-7dv5n8"> </td><td class="mono svelte-7dv5n8"> </td></tr>`);
var root_5$1 = from_html(`<div class="table-wrap svelte-7dv5n8"><table class="svelte-7dv5n8"><thead><tr><th class="sortable svelte-7dv5n8"> </th><th class="sortable right svelte-7dv5n8"> </th><!><th class="sortable right svelte-7dv5n8"> </th><!><!><!><!><th class="sortable svelte-7dv5n8"> </th><th class="svelte-7dv5n8">Staking Pool ID</th></tr></thead><tbody></tbody></table></div>`);
function ValidatorTable($$anchor, $$props) {
	...
	function toggleSort(key) {
		if (get(sortKey) === key) set(sortAsc, !get(sortAsc));
		else {
	...
	let sortedRows = user_derived(() => {
		...
		return [...$$props.rows].sort((a, b) => {
			...
			const bVal = b[key] ?? "";
			let cmp;
			if (isBigInt) cmp = Number(BigInt(String(aVal) || "0") - BigInt(String(bVal) || "0"));
			else if (isFloat) cmp = parseFloat(String(aVal) || "0") - parseFloat(String(bVal) || "0");
	...
	each(tbody, 21, () => get(sortedRows), index, ($$anchor, row) => {
		var tr_1 = root_4$1();
		var td = child(tr_1);
		var text_9 = child(td, true);
		reset(td);
		var td_1 = sibling(td);
		...
		var text_17 = child(td_8, true);
		reset(td_8);
		var td_9 = sibling(td_8);
		var text_18 = child(td_9, true);
		reset(td_9);
		reset(tr_1);
		template_effect(($0, $1) => {
		...
		}, [() => formatCommission(get(row).commissionRate), () => nanoToIotaFormatted(get(row).stakingPoolIotaBalance)]);
		append($$anchor, tr_1);
	});
	reset(tbody);
	reset(table);
...
delegate(["click"]);
//#endregion
//#region src/lib/pages/iota-system-state/IotaSystemState.svelte
var root = from_html(`<div> </div>`);
var root_1 = from_html(`<div class="view-toggle svelte-1aqomq7"><button>Table</button> <button>JSON</button></div>`);
var root_2 = from_html(`<tr><td class="kv-key svelte-1aqomq7"> </td><td class="kv-val mono svelte-1aqomq7"> </td></tr>`);
var root_3 = from_html(`<details class="section svelte-1aqomq7" open=""><summary class="svelte-1aqomq7"><h3 class="svelte-1aqomq7"> </h3></summary> <!></details>`);
...
var root_5 = from_html(`<tr><td class="kv-key svelte-1aqomq7"> </td><td class="kv-val mono svelte-1aqomq7"> </td><td class="kv-pct mono svelte-1aqomq7"> </td></tr>`);
var root_6 = from_html(`<div class="stake-info-wrap svelte-1aqomq7"><table class="kv-table svelte-1aqomq7"><tbody></tbody></table></div>`);
var root_7 = from_html(`<main><button class="svelte-1aqomq7">get latest IOTA system state</button> <button class="svelte-1aqomq7">candidate validators</button> <button class="svelte-1aqomq7">pending validators</button> <button class="svelte-1aqomq7">inactive validators</button> show full data (set before requesting): <select><option></option><option></option></select> <!> <!> <!> <!></main>`);
function IotaSystemState($$anchor, $$props) {
	...
	function formatFieldValue(key, val) {
		...
		if (nanoFields.has(key) && /^[\d_]+$/.test(str)) return nanoToIotaFormatted(str.replace(/_/g, "")) + " IOTA";
		return str;
	}
	...
	const getLatestSystemState = async () => {
		try {
			const result = await fetchLatestSystemState(getClient());
	...
	const getCandidateValidators = async () => {
		try {
			const { formattedValidators, stakeInfo: newStakeInfo, validatorRows: rows } = await fetchCandidateValidators(getClient(), get(showAllValidatorData));
	...
	const getPendingValidators = async () => {
		try {
			const { formattedValidators, stakeInfo: newStakeInfo, validatorRows: rows } = await fetchPendingValidators(getClient(), get(showAllValidatorData));
			set(value, formattedValidators);
			set(stakeInfo, newStakeInfo);
			set(validatorRows, rows);
			set(tableVariant, "candidate");
	...
	const getInactiveValidators = async () => {
		try {
			const { formattedValidators, validatorRows: rows } = await fetchInactiveValidators(getClient(), get(showAllValidatorData));
			set(value, formattedValidators);
			set(validatorRows, rows);
	...
	var consequent_5 = ($$anchor) => {
		...
		each(tbody, 5, () => Object.entries(get(baseFields)), index, ($$anchor, $$item) => {
		...
		var consequent_3 = ($$anchor) => {
			...
			reset(h3);
			reset(summary);
			ValidatorTable(sibling(summary, 2), {
				get rows() {
	...
	var consequent_7 = ($$anchor) => {
		...
		each(tbody_1, 5, () => Object.entries(get(stakeInfo)), index, ($$anchor, $$item) => {
			...
			template_effect(($0, $1) => {
				...
				set_text(text_7, $1);
			}, [() => nanoToIotaFormatted(stripUnderscores(val())), () => stakeFieldsWithPct.has(key()) ? percentOfSupply(val(), get(stakeInfo).totalSupply) : ""]);
			append($$anchor, tr_1);
		});
		reset(tbody_1);
		reset(table_1);
...
//#endregion
export { IotaSystemState as default };
