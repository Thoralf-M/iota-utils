import { $ as untrack, B as comment, D as set_class, Dt as pop, E as set_style, G as event, H as text, I as if_block, J as deep_read_state, Mt as reset, N as each, Ot as push, P as index, R as set_text, T as select_option, V as from_html, Y as get, ct as sibling, dt as mutate, ft as set, h as bind_value, i as prop, it as template_effect, jt as next, m as bind_group, n as onDestroy, nt as legacy_pre_effect_reset, ot as child, r as onMount, s as init, st as first_child, tt as legacy_pre_effect, u as bind_this, ut as mutable_source, v as remove_input_defaults, w as init_select, y as set_attribute, z as append } from "./disclose-version-CpEJO7r1.js";
...
import { i as TransactionView, o as copyToClipboard, s as formatAddress } from "./index-BmaCZbeO.js";
...
//#region src/lib/components/TransactionChart.svelte
var root$1 = from_html(`<option> </option>`);
var root_1$1 = from_html(`<button class="filter-btn svelte-1s1yw9c" title="Apply data filter to current zoom level for better performance">Filter Data</button>`);
var root_2$1 = from_html(`<button class="clear-filter-btn svelte-1s1yw9c" title="Show all data">Show All Data</button>`);
var root_3$1 = from_html(`<div class="selection-feedback svelte-1s1yw9c">📍 <strong>Selected:</strong> </div>`);
var root_4$1 = from_html(`<div class="data-filter-info svelte-1s1yw9c">📊 <strong>Data Filtered:</strong> </div>`);
var root_5$1 = from_html(`<div class="chart-container svelte-1s1yw9c"><div class="chart-controls svelte-1s1yw9c"><div class="control-group svelte-1s1yw9c"><label for="bucket-time" class="svelte-1s1yw9c">Time bucket:</label> <select id="bucket-time" class="svelte-1s1yw9c"></select></div> <div class="control-group svelte-1s1yw9c"><button class="reset-zoom-btn svelte-1s1yw9c" title="Reset zoom to show all data">Reset Zoom</button> <!> <!></div> <div class="zoom-instructions svelte-1s1yw9c"><small class="svelte-1s1yw9c">💡 Drag to select range, Ctrl+drag to pan, mouse wheel to zoom, click data point to
...
function TransactionChart($$anchor, $$props) {
	...
	function calculateBucketTime(data) {
		...
		if (totalTxs <= 100) return 1e4;
		else if (totalTxs <= 500) return 6e4;
		else if (totalTxs <= 1e3) return 6e5;
		else return 36e5;
	}
	function filterDataByZoom(data) {
		if (!zoomState.min || !zoomState.max) return data;
	...
	function selectCheckpoint(checkpointRange) {
		let firstCheckpoint;
		if (checkpointRange === "No transactions") return;
		if (checkpointRange.includes("-")) firstCheckpoint = checkpointRange.split("-")[0];
		else firstCheckpoint = checkpointRange;
		set(selectedCheckpoint, firstCheckpoint);
		if (onCheckpointSelected()) onCheckpointSelected()(firstCheckpoint);
		setTimeout(() => {
	...
	function createChart(preserveDataFilter = false) {
		...
		if (zoomState.min && zoomState.max) {
			if (zoomState.max - zoomState.min < get(bucketTime) * 2) shouldResetZoom = true;
		}
		...
		if (get(chart) && get(chart).scales && get(chart).scales.x && !shouldResetZoom && !isResettingZoom && dataToProcess.length > 0 && zoomState.min && zoomState.max) zoomState = {
			min: get(chart).scales.x.min,
			max: get(chart).scales.x.max
		};
		...
		if (needsReprocessing) {
			...
			if (get(bucketTime) >= 36e5) roundedStartTime.setMinutes(0);
			...
			if (get(bucketTime) <= 1e3) {
				sortedData.forEach((cp) => {
					const cpTime = new Date(cp.timestamp);
					const bucketKey = new Date(Math.floor(cpTime.getTime() / get(bucketTime)) * get(bucketTime)).getTime();
				...
				for (let i = 0; i < aggregatedData.length; i++) {
					...
					if (i < aggregatedData.length - 1) {
						const timeDiff = aggregatedData[i + 1].x.getTime() - current.x.getTime();
						if (timeDiff > get(bucketTime) * 3) {
				...
				sortedData.forEach((cp) => {
					...
					if (get(bucketTime) >= 36e5) bucketStart.setMinutes(0);
					else if (get(bucketTime) >= 6e5) {
				...
				if (bucketTimes.length > 0) {
					...
					if (get(bucketTime) <= 1e4) maxGapSize = get(bucketTime) * 5;
					else if (get(bucketTime) <= 6e4) maxGapSize = get(bucketTime) * 10;
					else maxGapSize = get(bucketTime) * 20;
					for (let time = firstBucket; time <= lastBucket; time += get(bucketTime)) {
						...
						if (hasData || isAtBoundary || isSignificantGap || fillAllBuckets) {
							let checkpointRange = "No transactions";
			...
			console.log(`Aggregation optimized (${bucketTimeLabel}): ${sortedData.length} transactions → ${aggregatedData.length} buckets`);
		}
		...
		if (get(bucketTime) <= 1e3) {
			...
			for (let i = 0; i < aggregatedData.length; i++) {
				...
				if (!(point.y === 0)) {
					...
					consecutiveZeros++;
					if (consecutiveZeros === 1 || consecutiveZeros % 10 === 0 || i === aggregatedData.length - 1) filteredAggregatedData.push(point);
				}
			}
		} else {
			const basicFiltered = [];
			...
			if (basicFiltered.length > 1e3) {
				...
				if (dataPointsPerHour > 360) samplingRate = 50;
				else if (dataPointsPerHour > 60) samplingRate = 100;
				else samplingRate = 200;
				let zeroCount = 0;
				for (let i = 0; i < basicFiltered.length; i++) {
					...
					if (!(point.y === 0)) {
						...
						if (isFirstZero || isLastZero || zeroCount % samplingRate === 0) filteredAggregatedData.push(point);
						zeroCount++;
					}
				}
			} else filteredAggregatedData = basicFiltered;
		}
		const bucketTimeLabel = get(bucketTime) < 1e3 ? `${get(bucketTime)}ms` : get(bucketTime) < 6e4 ? `${get(bucketTime) / 1e3}s` : `${get(bucketTime) / 6e4}min`;
		if (get(bucketTime) <= 1e3) console.log(`Filtering optimized (${bucketTimeLabel}): ${aggregatedData.length} → final: ${filteredAggregatedData.length} points (granular mode)`);
		else {
		...
		if (isResettingZoom || shouldResetZoom) if (get(isDataFiltered) && filterState.min && filterState.max) {
		...
		if (!get(chart) || needsReprocessing) {
			...
			set(chart, new auto_default(ctx, {
				type: "line",
				data: { datasets: [{
				...
				options: {
					...
					onClick: (event, activeElements) => {
						if (activeElements.length > 0) {
							...
							if (dataPoint && dataPoint.checkpointRange) selectCheckpoint(dataPoint.checkpointRange);
						}
					},
					plugins: {
						zoom: {
							...
							zoom: {
								...
								onZoomComplete({ chart }) {
									if (chart.scales && chart.scales.x) zoomState = {
						...
						tooltip: { callbacks: {
							title(context) {
								return `Checkpoints: ${context[0].raw.checkpointRange}`;
							},
							label(context) {
					...
					scales: {
						x: {
							...
							...zoomState.min && zoomState.max && !isResettingZoom && !shouldResetZoom ? {
								min: zoomState.min,
								max: zoomState.max
							} : {}
						},
	...
	legacy_pre_effect(() => (deep_read_state(checkpointData()), get(lastDataLength), get(userOverrideBucketTime), get(bucketTime)), () => {
		if (checkpointData().length > 0) {
			if (checkpointData().length !== get(lastDataLength)) {
				set(userOverrideBucketTime, false);
				set(lastDataLength, checkpointData().length);
			}
			if (!get(userOverrideBucketTime)) {
				const newBucketTime = calculateBucketTime(checkpointData());
				if (get(bucketTime) !== newBucketTime) set(bucketTime, newBucketTime);
			}
		}
	});
	legacy_pre_effect(() => (get(canvas), deep_read_state(checkpointData()), get(bucketTime)), () => {
		if (get(canvas) && (checkpointData().length > 0 || get(bucketTime))) createChart(false);
	});
	...
	each(select, 5, () => bucketOptions, index, ($$anchor, option) => {
		var option_1 = root$1();
		var text = child(option_1, true);
		reset(option_1);
		var option_1_value = {};
		template_effect(() => {
			set_text(text, (get(option), untrack(() => get(option).label)));
			if (option_1_value !== (option_1_value = (get(option), untrack(() => get(option).value)))) option_1.value = (option_1.__value = (get(option), untrack(() => get(option).value))) ?? "";
		});
	...
	var consequent_3 = ($$anchor) => {
		...
		reset(div_5);
		template_effect(($0) => set_text(text_2, ` Showing ${get(filteredDataLength) ?? ""} of ${get(originalDataLength) ?? ""} transactions
            (${$0 ?? ""}% of total data)`), [() => (get(filteredDataLength), get(originalDataLength), untrack(() => Math.round(get(filteredDataLength) / get(originalDataLength) * 100)))]);
		append($$anchor, div_5);
	};
	if_block(node_3, ($$render) => {
		if (get(isDataFiltered)) $$render(consequent_3);
	});
	bind_this(sibling(node_3, 2), ($$value) => set(canvas, $$value), () => get(canvas));
	reset(div);
	template_effect(() => {
		if (select_value !== (select_value = get(bucketTime))) select.value = (select.__value = get(bucketTime)) ?? "", select_option(select, get(bucketTime));
	});
...
//#endregion
//#region src/lib/pages/ptbs/ptb-service.ts
/**
...
function getPreviousCheckpoint(selectedCheckpoint, checkpointData) {
	...
	const current = parseInt(selectedCheckpoint);
	return checkpointData.filter((cp) => cp.transactionCount > 0).map((cp) => cp.sequenceNumber).sort((a, b) => a - b).filter((num) => num < current).pop()?.toString();
}
/**
...
function getNextCheckpoint(selectedCheckpoint, checkpointData) {
	if (!selectedCheckpoint) return;
	const current = parseInt(selectedCheckpoint);
	return checkpointData.filter((cp) => cp.transactionCount > 0).map((cp) => cp.sequenceNumber).sort((a, b) => a - b).find((num) => num > current)?.toString();
}
//#endregion
//#region src/lib/pages/ptbs/PTBs.svelte
var root = from_html(`<span class="checkpoint-count svelte-5fvkuj"> </span>`);
...
function PTBs($$anchor, $$props) {
	...
	function filterCheckpointTransactions() {
		...
		if (get(displayData).transactionsByCheckpoint && get(displayData).transactionsByCheckpoint.size > 0) {
			for (const txArr of get(displayData).transactionsByCheckpoint.values()) if (Array.isArray(txArr)) allTxs = allTxs.concat(txArr);
		} else allTxs = get(checkpointTransactions);
	...
	async function fetchEpochTransactionBlocks() {
		...
		} else if (!get(startCheckpoint) || !get(endCheckpoint) || get(startCheckpoint).toString().trim() === "" || get(endCheckpoint).toString().trim() === "") {
		...
		try {
			await analyzer.fetchAllTransactionBlocks(hasEpoch ? get(epoch) : void 0, hasCheckpointRange ? get(startCheckpoint) : void 0, hasCheckpointRange ? get(endCheckpoint) : void 0, (data, complete, processed, processedCp, totalCp) => {
			...
			}, get(inputObjectFilter) || void 0, get(functionFilter) || void 0);
		} catch (err) {
			if (get(stopRequested)) console.log("Fetch stopped by user request");
			else {
	...
	async function fetchLimitedTransactionBlocks() {
		...
		try {
			await analyzer.fetchLimitedTransactionBlocks(get(transactionLimit), hasEpoch ? get(epoch) : void 0, hasCheckpointRange ? get(startCheckpoint) : void 0, hasCheckpointRange ? get(endCheckpoint) : void 0, (data, complete, processed, processedCp, totalCp) => {
				set(displayData, data);
				set(processedTransactions, processed);
				set(processedCheckpoints, processedCp);
				set(totalCheckpoints, totalCp);
			...
			if (get(stopRequested)) console.log("Limited fetch stopped by user request");
			else {
				set(error, err.toString());
	...
	function onCheckpointInputChange() {
		...
		if (checkpointStr !== "") fetchCheckpointTransactions(checkpointStr);
		else {
			set(checkpointTransactions, []);
			set(filteredCheckpointTransactions, []);
			set(checkpointFilterSubstring, "");
	...
	function toggleTransactionIds(event) {
		const detailsElement = event.target;
		const transactionSection = detailsElement.parentElement.parentElement.parentElement.parentElement?.querySelector(".transaction-ids-section");
		if (transactionSection) if (detailsElement.open) transactionSection.classList.add("show");
		else transactionSection.classList.remove("show");
	}
	legacy_pre_effect(() => get(inputMode), () => {
		if (get(inputMode) === "epoch") {
			set(startCheckpoint, "");
			set(endCheckpoint, "");
		} else if (get(inputMode) === "checkpoint") set(epoch, "");
	});
	...
	var consequent_2 = ($$anchor) => {
		...
		var alternate = ($$anchor) => {
			append($$anchor, text("Get Current"));
		};
		if_block(node_1, ($$render) => {
		...
		var consequent_1 = ($$anchor) => {
			...
			reset(span);
			template_effect(($0) => set_text(text_2, `(${$0 ?? ""} checkpoints)`), [() => (get(epochCheckpointCount), untrack(() => get(epochCheckpointCount).toLocaleString()))]);
			append($$anchor, span);
		};
		if_block(node_2, ($$render) => {
	...
	var alternate_2 = ($$anchor) => {
		...
		var consequent_4 = ($$anchor) => {
			...
			reset(span_1);
			template_effect(($0) => set_text(text_5, `(${$0 ?? ""} checkpoints)`), [() => (get(checkpointRangeCount), untrack(() => get(checkpointRangeCount).toLocaleString()))]);
	...
	var text_7 = child(button_5, true);
	reset(button_5);
	reset(div_11);
	reset(div_1);
	...
	var consequent_11 = ($$anchor) => {
		...
		var consequent_10 = ($$anchor) => {
			...
			var text_15 = child(button_6, true);
			reset(button_6);
			reset(div_16);
	...
	var consequent_24 = ($$anchor) => {
		...
		var consequent_13 = ($$anchor) => {
			...
			reset(p_6);
			reset(div_22);
			template_effect(($0, $1, $2) => set_text(text_20, ` ${$0 ?? ""} - ${$1 ?? ""},
                        total: ${$2 ?? ""} checkpoints`), [
		...
		each(div_33, 5, () => (get(displayData), untrack(() => get(displayData).calledFunctionsList)), index, ($$anchor, func) => {
			var div_34 = root_13();
			var div_35 = child(div_34);
			var div_36 = child(div_35);
			var div_37 = child(div_36);
			var a = child(div_37);
			var text_27 = child(a, true);
			reset(a);
			var span_4 = sibling(a, 2);
			var text_28 = child(span_4, true);
			reset(span_4);
			var span_5 = sibling(span_4, 2);
			var text_29 = child(span_5, true);
			reset(span_5);
			...
			each(div_43, 5, () => (get(func), untrack(() => get(func).transactionIds)), index, ($$anchor, txId) => {
				var div_44 = root_12();
				var a_1 = child(div_44);
				var text_32 = child(a_1, true);
				reset(a_1);
				...
				template_effect(($0, $1) => {
					...
					set_text(text_32, $1);
				}, [() => (deep_read_state(getTransactionLink), deep_read_state(getSelectedNetworkConfig), get(txId), untrack(() => getTransactionLink(getSelectedNetworkConfig(), get(txId)))), () => (deep_read_state(formatAddress), get(txId), untrack(() => formatAddress(get(txId))))]);
		...
		each(div_45, 5, () => (get(displayData), untrack(() => get(displayData).uniqueSendersList)), index, ($$anchor, sender) => {
			var div_46 = root_14();
			var div_47 = child(div_46);
			var div_48 = child(div_47);
			var div_49 = child(div_48);
			var a_2 = child(div_49);
			var text_34 = child(a_2, true);
			reset(a_2);
			reset(div_49);
			...
			template_effect(($0, $1) => {
				...
				set_text(text_35, $1);
			}, [() => (deep_read_state(getAddressLink), deep_read_state(getSelectedNetworkConfig), get(sender), untrack(() => getAddressLink(getSelectedNetworkConfig(), get(sender).address))), () => (get(sender), untrack(() => get(sender).txCount.toLocaleString()))]);
		...
		each(div_52, 5, () => (get(displayData), untrack(() => get(displayData).calledPackagesList)), index, ($$anchor, pkg) => {
			var div_53 = root_15();
			var div_54 = child(div_53);
			var div_55 = child(div_54);
			var div_56 = child(div_55);
			var a_3 = child(div_56);
			var text_37 = child(a_3, true);
			reset(a_3);
			reset(div_56);
			...
			template_effect(($0, $1) => {
				...
				set_text(text_38, $1);
			}, [() => (deep_read_state(getObjectLink), deep_read_state(getSelectedNetworkConfig), get(pkg), untrack(() => getObjectLink(getSelectedNetworkConfig(), get(pkg).package))), () => (get(pkg), untrack(() => get(pkg).callCount.toLocaleString()))]);
		...
		each(div_59, 5, () => (get(displayData), untrack(() => get(displayData).publishedPackagesList)), index, ($$anchor, pkg) => {
			var div_60 = root_18();
			...
			var consequent_14 = ($$anchor) => {
				...
				each(div_66, 5, () => (get(pkg), untrack(() => get(pkg).modules)), index, ($$anchor, moduleName) => {
					var span_10 = root_16();
					var text_41 = child(span_10, true);
					reset(span_10);
		...
		var consequent_15 = ($$anchor) => {
			...
			each(first_child(fragment_4), 1, () => (get(displayData), untrack(() => get(displayData).commandTypeStats)), index, ($$anchor, cmd) => {
				var div_71 = root_19();
				...
				var text_45 = child(span_12, true);
				reset(span_12);
				reset(div_75);
				...
				var text_46 = child(span_13, true);
				reset(span_13);
				reset(div_78);
				reset(div_77);
				reset(div_73);
				...
				each(div_80, 5, () => (get(cmd), untrack(() => get(cmd).digests)), index, ($$anchor, txId) => {
					var div_81 = root_12();
					var a_7 = child(div_81);
					var text_47 = child(a_7, true);
					reset(a_7);
		...
		var consequent_23 = ($$anchor) => {
			...
			var consequent_21 = ($$anchor) => {
				...
				each(div_88, 5, () => (get(filteredCheckpointTransactions), get(checkpointFilterSubstring), get(checkpointTransactions), untrack(() => get(filteredCheckpointTransactions).length > 0 || get(checkpointFilterSubstring) ? get(filteredCheckpointTransactions) : get(checkpointTransactions))), index, ($$anchor, tx, index) => {
					var details_7 = root_24();
					var summary_5 = child(details_7);
					var span_14 = child(summary_5);
					span_14.textContent = `Transaction ${index + 1}`;
					var node_22 = sibling(span_14, 2);
					var consequent_18 = ($$anchor) => {
						var span_15 = root_22();
						var text_50 = child(span_15, true);
						reset(span_15);
						template_effect(() => {
					...
					var consequent_19 = ($$anchor) => {
						var span_16 = root_22();
						var text_51 = child(span_16, true);
						reset(span_16);
						template_effect(() => {
					...
					var consequent_20 = ($$anchor) => {
						var span_17 = root_23();
						let classes;
						var text_52 = child(span_17, true);
						reset(span_17);
						template_effect(() => {
					...
					TransactionView(child(div_89), { get value() {
						return get(tx);
					} });
					reset(div_89);
					reset(details_7);
					append($$anchor, details_7);
				});
				reset(div_88);
				reset(div_87);
			...
			}, [() => (get(selectedCheckpoint), deep_read_state(hasPreviousCheckpoint), get(displayData), untrack(() => !get(selectedCheckpoint) || !hasPreviousCheckpoint(get(selectedCheckpoint), get(displayData).checkpointData))), () => (get(selectedCheckpoint), deep_read_state(hasNextCheckpoint), get(displayData), untrack(() => !get(selectedCheckpoint) || !hasNextCheckpoint(get(selectedCheckpoint), get(displayData).checkpointData)))]);
