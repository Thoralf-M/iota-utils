import { $ as untrack, D as set_class, Dt as pop, I as if_block, Mt as reset, N as each, Ot as push, P as index, R as set_text, U as delegate, V as from_html, W as delegated, Y as get, _t as remove_textarea_child, ct as sibling, ft as set, h as bind_value, it as template_effect, nt as legacy_pre_effect_reset, ot as child, s as init, tt as legacy_pre_effect, ut as mutable_source, vt as setup_stores, x as set_selected, yt as store_get, z as append } from "./disclose-version-CpEJO7r1.js";
...
import { n as addAndRun } from "./index-DDlV3uWb.js";
...
var root_3 = from_html(`<div style="margin-top: 0.5rem; font-size: 0.9rem; color: #666;">No coins available</div>`);
...
0x111173a14c3d402c01546c54265c30cc04414c7b7ec1732412bb19066dd49d11 2000000000`);
	...
	let hasFetched = mutable_source(false);
	const fetchAvailableCoins = async () => {
		try {
			set(fetchingCoins, true);
			set(fetchError, "");
			const balances = await getClient().getAllBalances({ owner: $activeAddress() });
	...
	function extractSymbolFromCoinType(coinType) {
		if (coinType === "0x2::iota::IOTA") return "IOTA";
		else {
	...
	function parseTransfers(input) {
		...
		try {
			...
			if (Array.isArray(parsed)) {
				for (let i = 0; i < parsed.length; i++) {
					...
					if (!transfer.address || typeof transfer.address !== "string") throw new Error(`Transfer ${i}: address is required and must be a string`);
					if (!isValidIotaAddress(transfer.address)) throw new Error(`Transfer ${i}: invalid IOTA address "${transfer.address}"`);
					if (!transfer.amount || typeof transfer.amount !== "number" || transfer.amount <= 0) throw new Error(`Transfer ${i}: amount is required and must be a positive number`);
				}
				return parsed;
			}
		} catch (e) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (i === 0 && line.toLowerCase().includes("address") && line.toLowerCase().includes("amount")) continue;
			let parts = line.split(",").map((p) => p.trim());
			if (parts.length !== 2) parts = line.split(/\s+/);
			if (parts.length !== 2) throw new Error(`Line ${i + 1}: Expected format "address,amount" or "address amount", got "${line}"`);
			const [address, amountStr] = parts;
			if (!isValidIotaAddress(address)) throw new Error(`Line ${i + 1}: Invalid IOTA address "${address}"`);
			const amount = parseFloat(amountStr);
			if (isNaN(amount) || amount <= 0) throw new Error(`Line ${i + 1}: Invalid amount "${amountStr}"`);
			transfers.push({
				address,
				amount
			});
	...
	const executeBulkTransfer = async () => {
		try {
			...
			if (get(coinType) === "0x2::iota::IOTA") {
				...
				let availableCoins = await client.getCoins({
					owner: iotaAddress,
					coinType: get(coinType)
				});
				if (availableCoins.data.length === 0) throw new Error(`No ${get(coinSymbol)} coins available for transfer`);
				let selectedAmount = BigInt(0);
				let selectedCoins = [];
				for (const coin of availableCoins.data) {
					if (selectedAmount >= totalTransferAmount) break;
					selectedAmount += BigInt(coin.balance);
					selectedCoins.push(coin);
				}
				if (selectedAmount < totalTransferAmount) throw new Error(`Not enough ${get(coinSymbol)} coins available for transfer. Available: ${selectedAmount}, Required: ${totalTransferAmount}`);
				const coinOne = txb.object(selectedCoins.shift().coinObjectId);
				if (selectedCoins.length > 0) txb.mergeCoins(coinOne, selectedCoins.map((coin) => txb.object(coin.coinObjectId)));
			...
			await addAndRun({
				label: `Bulk transfer ${transfers.length} × ${get(coinSymbol)}`,
				transaction: txb,
	...
	function handleJsonChange() {
		try {
			...
			if (transfers.length === 0) throw new Error("Transfers array cannot be empty");
			for (let i = 0; i < transfers.length; i++) {
				const transfer = transfers[i];
				if (!transfer.address || typeof transfer.address !== "string") throw new Error(`Transfer ${i}: address is required and must be a string`);
				if (!isValidIotaAddress(transfer.address)) throw new Error(`Transfer ${i}: invalid IOTA address "${transfer.address}"`);
	...
	legacy_pre_effect(() => get(coinType), () => {
		if (get(coinType) === "0x2::iota::IOTA") set(coinSymbol, "IOTA");
		else {
	...
	legacy_pre_effect(() => get(transfersJson), () => {
		set(totalAmountNano, (() => {
			try {
				return parseTransfers(get(transfersJson)).reduce((sum, transfer) => sum + transfer.amount, 0);
			} catch {
				return 0;
			}
		})());
	...
	var consequent_1 = ($$anchor) => {
		...
		each(sibling(option), 1, () => get(availableCoins), index, ($$anchor, coin) => {
			var option_1 = root_1();
			...
			template_effect(($0) => {
				...
				if (option_1_value !== (option_1_value = (get(coin), untrack(() => get(coin).coinType)))) option_1.value = (option_1.__value = (get(coin), untrack(() => get(coin).coinType))) ?? "";
	...
	var consequent_4 = ($$anchor) => {
		...
		template_effect(($0) => {
			...
			try {
				return parseTransfers(get(transfersJson)).length;
			} catch {
				return 0;
			}
		})()))]);
...
//#endregion
export { BulkTransfer as default };
