import { Dt as pop, I as if_block, Mt as reset, Ot as push, R as set_text, U as delegate, V as from_html, W as delegated, Y as get, ct as sibling, dt as mutate, ft as set, h as bind_value, it as template_effect, ot as child, r as onMount, s as init, u as bind_this, ut as mutable_source, v as remove_input_defaults, vt as setup_stores, yt as store_get, z as append } from "./disclose-version-CpEJO7r1.js";
import "./legacy-DxVWxrJw.js";
import { r as getSelectedNetworkConfig, s as TransactionDataBuilder, t as getClient } from "./client-BTFoHz6u.js";
...
function Transaction($$anchor, $$props) {
	...
	function buildFilter() {
		...
		if (get(inputObjectFilter).trim()) filter.inputObject = get(inputObjectFilter).trim();
		if (get(functionFilter).trim()) filter.function = get(functionFilter).trim();
		return filter;
	}
	function hasFilters() {
	...
	function detectInputType(input) {
		...
		if (/^[1-9A-HJ-NP-Za-km-z]{43,44}$/.test(trimmed)) return "base58";
	...
	async function fetchTransactionByDigest(digest) {
		try {
			...
			if (tx.rawTransaction) try {
				const signedBytes = fromBase64(tx.rawTransaction);
				const v1Data = iotaBcs.SenderSignedData.parse(signedBytes)[0].intentMessage.value.V1;
				...
				} else mutate(transactionData, get(transactionData).transactionBytes = tx.rawTransaction);
			} catch (e) {
		...
		try {
			...
			if (type === "json") {
				...
				try {
					...
					if (jsonData && jsonData.intentMessage && jsonData.txSignatures) set(transactionData, getTransactionData(jsonData));
					else throw new Error("Not a signed transaction JSON format");
				} catch (e2) {
					...
					try {
						...
						if (v1Data.kind && v1Data.kind.ProgrammableTransaction) {
							const transactionBytesForDryRun = toBase64(new TransactionDataBuilder({
								version: 2,
								sender: v1Data.sender,
								inputs: v1Data.kind.ProgrammableTransaction.inputs,
								commands: v1Data.kind.ProgrammableTransaction.commands,
						...
						} else throw new Error("Unsupported transaction kind");
					} catch (e3) {
	...
	async function fetchLatestPTB() {
		try {
			...
			set(error, "");
			const result = await new IotaGraphQLClient({ url: getSelectedNetworkConfig().graphql }).query({
				query: `
			...
			set(hasNext, false);
			txInput = digest;
			if (get(txBytesTextarea)) mutate(txBytesTextarea, get(txBytesTextarea).value = digest);
	...
	function handleInput(event) {
		...
		if (txInput.trim()) inputTimeout = setTimeout(() => {
			processInput();
		}, 500);
		else {
			set(transactionData, null);
	...
	delegated("click", button, fetchLatestPTB);
	delegated("click", button_1, fetchPreviousPTB);
	delegated("click", button_2, fetchNextPTB);
	delegated("click", button_3, () => set(showFilters, !get(showFilters)));
	delegated("click", button_4, () => loadExample(exampleTx));
	delegated("click", button_5, () => loadExample(exampleSignedTx));
	append($$anchor, div);
	pop();
	$$cleanup();
}
