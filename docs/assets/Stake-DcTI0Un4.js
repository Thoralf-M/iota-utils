import { $ as untrack, B as comment, Dt as pop, E as set_style, G as event, I as if_block, J as deep_read_state, Mt as reset, N as each, Ot as push, P as index, R as set_text, V as from_html, Y as get, ct as sibling, ft as set, gt as user_derived, h as bind_value, it as template_effect, nt as legacy_pre_effect_reset, ot as child, s as init, st as first_child, tt as legacy_pre_effect, ut as mutable_source, v as remove_input_defaults, vt as setup_stores, yt as store_get, z as append } from "./disclose-version-CpEJO7r1.js";
...
import { _ as nanoToIotaFormatted, i as TransactionView, m as formatNumbersWithUnderscores, n as addAndRun, p as formatNumberWithUnderscores } from "./index-DDlV3uWb.js";
import { t as IotaAmountInput } from "./IotaAmountInput-bLJmSlYx.js";
...
async function buildUnstakeSingleTransaction(client, stakedIotaObjectId) {
	...
	let target;
	if (obj.data?.content?.type === "0x3::staking_pool::StakedIota") target = "0x3::iota_system::request_withdraw_stake";
	if (obj.data?.content?.type === "0x3::timelocked_staking::TimelockedStakedIota") target = "0x3::timelocked_staking::request_withdraw_stake";
...
*/
async function computeRequiredUnstakeAmount(client, stakedIotaObjectId, targetAmount, activeAddress) {
	...
	if (obj.data?.content?.type === "0x3::timelocked_staking::TimelockedStakedIota") timelocked = true;
	if (!timelocked && obj.data?.content?.type !== "0x3::staking_pool::StakedIota") throw new Error("No staked IOTA object: " + stakedIotaObjectId);
	...
	if (timelocked) initialUnstakeAmount = (targetAmount * initialStaked + rewards - 1n) / rewards;
	else initialUnstakeAmount = (targetAmount * initialStaked + (initialStaked + rewards - 1n)) / (initialStaked + rewards);
	return {
...
*/
async function devInspectStakedObject(client, stakedIotaObjectId, activeAddress) {
...
function buildUnstakeAllTransaction(stakedIota, timelockedStakedIota, targetAddress) {
	const tx = new Transaction();
	let firstBalance;
	for (const [index, delegatedStake] of stakedIota.entries()) for (const [innerIndex, stake] of delegatedStake.stakes.entries()) {
		const balance = tx.moveCall({
			target: "0x3::iota_system::request_withdraw_stake_non_entry",
			arguments: [tx.object("0x5"), tx.object(stake.stakedIotaId)]
		});
		if (index === 0 && innerIndex === 0) firstBalance = balance;
		else tx.moveCall({
	...
	for (const [index, delegatedTimelockedStake] of timelockedStakedIota.entries()) for (const [innerIndex, timelockedStake] of delegatedTimelockedStake.stakes.entries()) {
		const [timelock, balance] = tx.moveCall({
			target: "0x3::timelocked_staking::request_withdraw_stake_non_entry",
		...
		if (index === 0 && innerIndex === 0 && !firstBalance) firstBalance = balance;
		else tx.moveCall({
			target: "0x2::balance::join",
			arguments: [firstBalance, balance],
			typeArguments: ["0x2::iota::IOTA"]
...
async function getTimelockedObjects(client, address) {
	const ownedObjectPage = await client.getOwnedObjects({
		owner: address,
		filter: { StructType: "0x2::timelock::TimeLock<0x2::balance::Balance<0x2::iota::IOTA>>" },
		options: { showContent: true }
	});
	if (ownedObjectPage.data.length === 0) throw new Error("no timelocked object found");
	return ownedObjectPage.data.map((d) => d.data);
}
/**
* Run a simulation to test different unstake amounts
*/
async function unstakeSpecificAmountSimulation(client, stakedIotaObjectId, targetAmount, activeAddress) {
	const { amount: initialUnstakeAmount, timelocked } = await computeRequiredUnstakeAmount(client, stakedIotaObjectId, targetAmount, activeAddress);
	const results = [];
	for (const diff of [
		-1000000000n,
		-1000n,
		...
		const tx = buildSingleObjectUnstakeTransaction(stakedIotaObjectId, unstakeAmount, timelocked, activeAddress);
		...
		if (unstakeAmount === initialUnstakeAmount) results.push(resString + " this would be used");
		else results.push(resString);
	}
	return results;
}
...
//#endregion
//#region src/lib/pages/stake/validator-service.ts
/**
...
function getCommitteeMemberAddresses(systemState) {
	...
	if (systemState.committeeMembers && Array.isArray(systemState.committeeMembers)) systemState.committeeMembers.forEach((validator) => {
...
async function fetchValidatorByAddress(client, address) {
	...
	try {
		...
		for (const validator of systemState.activeValidators) if (validator.iotaAddress === address) {
		...
		while (hasNextPage) {
			...
			for (const candidateValidator of candidateValidatorsPage.data) try {
				...
				const validator = (await client.getObject({
					id: validatorV1.data[0].objectId,
					options: { showContent: true }
				})).data?.content.fields.value.fields;
				if (validator.metadata.fields.iota_address === address) return {
			...
			if (hasNextPage) nextPageCursor = candidateValidatorsPage.nextCursor;
		}
		return null;
	} catch (err) {
...
function Stake($$anchor, $$props) {
	...
	const stake = async () => {
		try {
			if (!isValidIotaAddress(get(validatorAddress))) throw new Error("invalid address");
			const tx = buildStakeTransaction(get(validatorAddress), get(amount));
			await addAndRun({
				label: `Stake ${nanoToIotaFormatted(String(get(amount)))} IOTA → ${shortAddr(get(validatorAddress))}`,
				transaction: tx,
				recipients: [get(validatorAddress)]
			});
		} catch (err) {
			console.error(err);
			alert(err.toString());
	...
	async function unstakeSingle() {
		try {
			...
			await addAndRun({
				label: `Unstake ${shortAddr(get(stakedIotaObjectId))}`,
				transaction: tx
			});
		} catch (err) {
			console.error(err);
	...
	async function computeRewards(stakedIotaObjectId) {
		try {
			let stakeData = await devInspectStakedObject(getClient(), stakedIotaObjectId, $activeAddress());
	...
	async function unstakeSpecificAmountSim(stakedIotaObjectId) {
		try {
			const results = await unstakeSpecificAmountSimulation(getClient(), stakedIotaObjectId, BigInt(get(amount)), $activeAddress());
			set(value, results);
		} catch (err) {
	...
	async function unstakeSpecificAmount(stakedIotaObjectId) {
		try {
			const { amount: initialUnstakeAmount, timelocked } = await computeRequiredUnstakeAmount(getClient(), stakedIotaObjectId, BigInt(get(amount)), $activeAddress());
			const tx = buildSingleObjectUnstakeTransaction(stakedIotaObjectId, initialUnstakeAmount, timelocked, $activeAddress());
			await addAndRun({
				label: `Unstake ${nanoToIotaFormatted(String(get(amount)))} IOTA from ${shortAddr(stakedIotaObjectId)}`,
				transaction: tx
			});
		} catch (err) {
			console.error(err);
	...
	const handleLoadValidators = async () => {
		try {
			...
			while (hasNextPage) {
				...
				for (const candidateValidator of candidateValidatorsPage.data) try {
					const innerId = (await client.getDynamicFieldObjectV2({
						parentObjectId: validatorCandidatesId,
						name: candidateValidator.name,
						options: { showContent: true }
					})).data.content.fields.value.fields.inner.fields.id.id;
	...
	const handleFetchValidatorByAddress = async (address) => {
		...
		if (validator && get(validatorAddress) === validator.address) set(selectedValidator, validator);
	};
	async function unstakeAll() {
		try {
			...
			const stakeCount = staked.stakedIota.reduce((n, ds) => n + ds.stakes.length, 0) + staked.timelockedStakedIota.reduce((n, ds) => n + ds.stakes.length, 0);
			await addAndRun({
	...
	const stakeAllTimelockedObjects = async () => {
		try {
			...
			for (const timelockedObject of timelockedObjects) tx.moveCall({
			...
			await addAndRun({
				label: `Stake ${timelockedObjects.length} timelocked object${timelockedObjects.length === 1 ? "" : "s"} → ${shortAddr(get(validatorAddress))}`,
				transaction: tx,
				recipients: [get(validatorAddress)]
			});
		} catch (err) {
	...
	async function listStakedIota() {
		try {
			...
			if (stakedIota.length == 0 && timelockedStakedIota.length == 0) throw new Error("no staked IOTA found");
			if (stakedIota.length != 0) set(stakedIotaObjectId, stakedIota[0].stakes[0].stakedIotaId);
			else set(stakedIotaObjectId, timelockedStakedIota[0].stakes[0].timelockedStakedIotaId);
			for (let delegatedStake of stakedIota) for (let stake of delegatedStake.stakes) {
				let stakeData = await devInspectStakedObject(getClient(), stake.stakedIotaId, $activeAddress());
				...
				for (const key in formattedStake) stake[key] = formattedStake[key];
			}
			for (let delegatedTimelockedStake of timelockedStakedIota) for (let timelockedStake of delegatedTimelockedStake.stakes) {
				...
				for (const key in formattedTimelockedStake) timelockedStake[key] = formattedTimelockedStake[key];
			}
			let res = {
				stakedIota,
	...
	legacy_pre_effect(() => (get(validatorAddress), isValidIotaAddress), () => {
		if (get(validatorAddress) && isValidIotaAddress(get(validatorAddress))) {
			...
			if (foundValidator) set(selectedValidator, foundValidator);
			else handleFetchValidatorByAddress(get(validatorAddress));
		} else set(selectedValidator, null);
	});
	...
	var consequent = ($$anchor) => {
		...
		reset(pre);
		reset(div);
		template_effect(($0) => set_text(text, $0), [() => (get(devInspectValue), untrack(() => JSON.stringify(get(devInspectValue), null, 2)))]);
		append($$anchor, div);
	};
	...
	var consequent_6 = ($$anchor) => {
		...
		var alternate = ($$anchor) => {
			...
			var consequent_3 = ($$anchor) => {
				...
				each(first_child(fragment_1), 1, () => (get(validators), untrack(() => get(validators).filter((v) => v.status === "Committee Member"))), index, ($$anchor, validator) => {
					var div_4 = root_3();
			...
			var consequent_4 = ($$anchor) => {
				...
				each(first_child(fragment_2), 1, () => (get(validators), untrack(() => get(validators).filter((v) => v.status === "Active Validator"))), index, ($$anchor, validator) => {
					var div_8 = root_4();
			...
			var consequent_5 = ($$anchor) => {
				...
				each(first_child(fragment_3), 1, () => (get(validators), untrack(() => get(validators).filter((v) => v.status === "Candidate"))), index, ($$anchor, validator) => {
					var div_12 = root_5();
	...
	var consequent_7 = ($$anchor) => {
		...
		var text_22 = child(span_22, true);
		reset(span_22);
	...
	TransactionView(sibling(button_11, 2), { get value() {
		return get(value);
	} });
	reset(main);
